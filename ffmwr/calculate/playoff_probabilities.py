                    ...
                    while sim_count <= self.simulations:
                        ...
                        for wk, matchups in remaining_matchups.items():
                            for matchup in matchups:
                                ...
                                # Instead of: result = int(random.getrandbits(1))  # 50/50 coin flip
                                # Use team strength based on points scored:
                                team_1_strength = team_1.points_for
                                team_2_strength = team_2.points_for

                                # Calculate win probability for team_1
                                if team_1_strength + team_2_strength > 0:
                                    team_1_win_prob = team_1_strength / (team_1_strength + team_2_strength)
                                else:
                                    team_1_win_prob = 0.5

                                # Simulate with weighted probability
                                result = 1 if random.random() < team_1_win_prob else 0
                                if result == 1:
                                    team_1.add_win()
                                    team_2.add_loss()
                        ...
                        if self.num_divisions > 0:
                            ...
                            if (len(division_winners) + len(division_qualifiers)) < self.num_playoff_slots:
                                remaining_playoff_count = 1
                                max_remaining_slots = self.num_playoff_slots - len(division_winners) - len(division_qualifiers)
                                
                                # FIX: Ensure we don't exceed the number of remaining teams
                                available_remaining_teams = min(len(remaining_teams), max_remaining_slots)
                                
                                while remaining_playoff_count <= available_remaining_teams:
                                    # FIX: Add additional bounds check
                                    if remaining_playoff_count - 1 < len(remaining_teams):
                                        teams_for_playoff_probs[
                                        ...
                                        logger.warning(
                                            f"Skipping remaining playoff slot {remaining_playoff_count}: "
                                            f"Not enough remaining teams ({len(remaining_teams)} available)"
                                        )
                                    remaining_playoff_count += 1
