...
class ReportData(object):
    def __init__(
        ...
        week_teams_for_counter = league.teams_by_week.get(str(week_counter))
        if not week_teams_for_counter:
            logger.warning(f"No teams found for week {week_counter}")
            week_teams_for_counter = {}
        
        self.teams_results = {
            team.team_id: add_report_team_stats(
                settings, team, league, week_counter, metrics_calculator, metrics, dq_ce, inactive_players
            )
            for team in week_teams_for_counter.values()
        }
        ...
        # Only include teams that have records for this week
        week_teams = [
            team for team in week_teams_for_counter.values()
            if str(team.team_id) in league.records_by_week.get(str(week_counter), {})
        ]
        ...
        if create_z_score_data:
            for k_v in sorted(z_score_results.items(), key=lambda x: x[1], reverse=True):
                ...
                # Only include teams that exist in teams_results
                if k_v[0] not in self.teams_results:
                    continue
