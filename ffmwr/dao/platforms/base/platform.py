...
class BasePlatform(ABC):
    ...
    def get_mapped_position(self, platform_position: str) -> str:
        mapped = self.position_mapping.get(platform_position)
        if not mapped:
            logger.warning(f"Unknown position in mapping: {platform_position}, returning as-is")
            return platform_position
        return mapped.get("base")
