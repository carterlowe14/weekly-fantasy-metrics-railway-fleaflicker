...
class FleaflickerPlatform(BasePlatform):
    ...
    def map_data_to_base(self) -> None:
        ...
        # Note: Scraping the current week from HTML is unreliable due to page structure changes.
        # We rely on the current_week passed via get_current_nfl_week_function instead.
        ...
        for elem in elements:
            if elem.text.strip() == "Playoffs":
                ...
                if any((text.strip() and "Weeks" in text) for text in playoff_weeks_elements):
                    ...
                    self.league.num_regular_season_weeks = 17 if int(self.league.season) >= 2021 else 16
                else:
        ...
        for week, rosters in rosters_by_week.items():
            ...
            for team_id, roster in rosters.items():
                week_teams = self.league.teams_by_week.get(str(week))
                if not week_teams:
                    logger.warning(f"No teams found for week {week}, skipping roster processing for this week")
                    continue
                
                league_team: BaseTeam = week_teams.get(str(team_id))
                if not league_team:
