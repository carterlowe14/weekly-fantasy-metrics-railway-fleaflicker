...
def user_week_input_validation(settings: AppSettings, week: int, retrieved_current_week: int, season: int) -> int:
    ...
    # If running with use_default flag, skip all prompts
    if os.getenv('USE_DEFAULT') == '1':
        if week_for_report == "default":
            if (int(retrieved_current_week) - 1) > 0:
                week_for_report = int(retrieved_current_week) - 1
            else:
                week_for_report = retrieved_current_week
        return int(week_for_report)
    
    # Handle "default" for past seasons - use the last week of the season
    if week_for_report == "default" and not (current_year == int(season) or (current_year == (int(season) + 1) and current_month < 9)):
        week_for_report = settings.nfl_season_length - 1
