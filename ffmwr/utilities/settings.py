...
def get_app_settings_from_env_file(env_file_path: Path) -> AppSettings:
    ...
    env_vars_present = any(os.getenv(str(field[0])) is not None for field in env_fields)
    if env_vars_present:
        logger.debug('Local ".env" file not found. Loading Fantasy Football Metrics Weekly Report settings from environment variables.')
        return AppSettings()

    logger.debug('Local ".env" file not found and no environment variables are set.')
